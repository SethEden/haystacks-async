...
async function getWorkflow(workflowName) {
  ...
  let currentWorkflow = await searchWorkflow(
    D[sys.cCommandWorkflows],
    workflowName,
  );
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.ccurrentWorkflowIs + JSON.stringify(currentWorkflow),
  );
  workflowValue = currentWorkflow;
  // workflowValue is:
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cworkflowValueIs + JSON.stringify(workflowValue),
  );
  await loggers.consoleLog(namespacePrefix + functionName, msg.cEND_Function);
...
async function doesWorkflowExist(workflowName) {
  ...
  let workflowSearchResult = await searchWorkflow(
    D[sys.cCommandWorkflows],
    workflowName,
  );
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cworkflowFoundIs + workflowFound,
  );
  await loggers.consoleLog(namespacePrefix + functionName, msg.cEND_Function);
...
async function searchWorkflow(workflowData, workflowName) {
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cworkflowDataIs + JSON.stringify(workflowData),
  );
  // workflowName is:
  await loggers.consoleLog(
    namespacePrefix + functionName,
  ...
  if (typeof workflowData === wrd.cobject) {
    for (let workflowEntity in workflowData) {
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cworkflowIs + JSON.stringify(workflowData[workflowEntity]),
      );
      if (
        workflowEntity != workflowName ||
        (workflowEntity === workflowName &&
          typeof workflowData[workflowEntity] === wrd.cobject)
      ) {
        let workflowObjectTemp = await searchWorkflow(
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cworkflowObjectIs + workflowObject,
  );
  await loggers.consoleLog(namespacePrefix + functionName, msg.cEND_Function);
...
async function getAllWorkflows(workflowDataStructure) {
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cworkflowDataStructureIs + JSON.stringify(workflowDataStructure),
  );
  let allWorkflows = false;
  let internalWorkflowDataStructure;
  if (workflowDataStructure === undefined) {
    internalWorkflowDataStructure = JSON.parse(
      JSON.stringify(D[sys.cCommandWorkflows]),
    );
  } else {
    internalWorkflowDataStructure = JSON.parse(
      JSON.stringify(workflowDataStructure),
    );
  }
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cinternalWorkflowDataStructureIs +
      JSON.stringify(internalWorkflowDataStructure),
  );
  if (typeof internalWorkflowDataStructure === wrd.cobject) {
    ...
    for (let workflowEntity in internalWorkflowDataStructure) {
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cinternalWorkflowDataStructureWorkflowEntityIs +
          JSON.stringify(internalWorkflowDataStructure[workflowEntity]),
      );
      if (
        typeof internalWorkflowDataStructure[workflowEntity] === wrd.cobject
      ) {
        // internalWorkflowDataStructure[workflowEntity] is of type object!
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.cinternalWorkflowDataStructureWorkflowEntityIsOfTypeObject,
        );
        let allWorkflowsTemp;
        allWorkflowsTemp = await getAllWorkflows(
          internalWorkflowDataStructure[workflowEntity],
        );
        // allWorkflowsTemp returned from the recursive call is:
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.callWorkflowsTempReturnedFromRecursiveCallIs +
            JSON.stringify(allWorkflowsTemp),
        );
        if (allWorkflowsTemp === false) {
          // The recursive call returned false, so push the current entity to the output array!
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cgetAllWorkflowsMessage01 + msg.cgetAllWorkflowsMessage02,
          );
          allWorkflows.push(workflowEntity);
          // allWorkflows after pushing to the array 1 is:
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.callWorkflowsAfterPushingToArray1Is +
        ...
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.cgetAllWorkflowsMessage03 + msg.cgetAllWorkflowsMessage04,
        );
        allWorkflows.push(workflowEntity);
        // allWorkflows after pushing to the array 2 is:
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.callWorkflowsAfterPushingToArray2Is +
            JSON.stringify(allWorkflows),
        );
      }
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cworkflowIs + JSON.stringify(allWorkflows),
  );
  await loggers.consoleLog(namespacePrefix + functionName, msg.cEND_Function);
  return allWorkflows;
}
 ...
 * @date 2022/05/25
 */
async function getWorkflowNamespaceDataObject(
  ...
  for (let workflowEntity in workflowDataStructure) {
    ...
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cworkflowDataStructureWorkflowEntityIs +
        JSON.stringify(workflowDataStructure[workflowEntity]),
    );
    if (workflowEntity === namespaceToFind) {
      ...
      // Search recursively
      let workflowTempObject = await getWorkflowNamespaceDataObject(
        workflowDataStructure[workflowEntity],
        namespaceToFind,
      );
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cworkflowNamespaceObjectIs + JSON.stringify(workflowNamespaceObject),
  );
  await loggers.consoleLog(namespacePrefix + functionName, msg.cEND_Function);
