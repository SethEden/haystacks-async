...
async function addPluginCommands(pluginName, pluginCommands) {
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cpluginCommandsIs + JSON.stringify(pluginCommands),
  );
...
async function getValidCommand(commandString, commandDelimiter) {
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.ccommandStringIs + commandString,
  );
  // commandDelimiter is:
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.ccommandDelimiterIs + commandDelimiter,
  );
  let returnData = false;
  ...
  if (
    ...
    commandDelimiter === undefined
  ) {
    commandArgsDelimiter = bas.cSpace;
  }
  if (
    commandString &&
    typeof commandString === wrd.cstring &&
    commandString.includes(commandArgsDelimiter) === true
  ) {
  ...
  if (commandString) {
    if (D[wrd.cCommands][commandToExecute] != undefined) {
      ...
      // else-clause if (D[wrd.cCommands][commandToExecute] != undefined)
      // else-clause looking for command aliases.
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.celseClauseLookingForCommandAliases,
      );
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.callCommandAliasesIs + JSON.stringify(allCommandAliases),
      );
      // Search through the data structure recursively to see if we can find the command or command alias.
      foundValidCommand = await searchCommandAlias(
        allCommandAliases,
        commandToExecute,
      );
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cfoundValidCommandIs + JSON.stringify(foundValidCommand),
      );
      ...
      if (foundValidCommand === false) {
        ...
        console.log(
          msg.cWarningTheSpecifiedCommand +
            commandToExecute +
            msg.cdoesNotExistPleaseTryAgain +
            bas.cSpace +
            num.c1,
        );
      } else {
...
async function countMatchingCommandAlias(commandAliasData, commandAliasName) {
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.ccommandAliasDataIs + JSON.stringify(commandAliasData),
  );
  // commandAliasName is:
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.ccommandAliasNameIs + commandAliasName,
  );
  ...
  if (typeof commandAliasData === wrd.cobject) {
    for (let commandAliasEntity in commandAliasData) {
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.ccommandAliasEntityIs + JSON.stringify(commandAliasEntity),
      );
      // commandAliasEntityValue is:
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.ccommandAliasEntityValueIs +
          JSON.stringify(commandAliasData[commandAliasEntity]),
      );
      if (commandAliasEntity.toUpperCase() != commandAliasName.toUpperCase()) {
        if (
          commandAliasData[commandAliasEntity] != undefined &&
          commandAliasData[commandAliasEntity][wrd.cAliases] != undefined
        ) {
          ...
          if (commandAliasData[commandAliasEntity] != undefined) {
            let tempCommandAliasCount = await countMatchingCommandAlias(
              commandAliasData[commandAliasEntity],
              commandAliasName,
            );
            // tempCommandAliasCount is:
            await loggers.consoleLog(
              namespacePrefix + functionName,
              msg.ctempCommandAliasCountIs + tempCommandAliasCount,
            );
            if (tempCommandAliasCount > 0) {
              // adding commandAliasCount:
              await loggers.consoleLog(
                namespacePrefix + functionName,
                msg.caddingCommandAliasCount + commandAliasCount,
              );
              commandAliasCount = commandAliasCount + tempCommandAliasCount;
              // After adding commandAliasCount and tempCommandAliasCount:
              await loggers.consoleLog(
                namespacePrefix + functionName,
                msg.cAfterAddingCommandAliasCountAndTempCommandAliasCount +
                  commandAliasCount,
              );
              // Don't break, continue searching, so we get a full count of any duplicates found.
            ...
            let errorMessage =
              msg.cErrorCountMatchingCommandAliasMessage01 +
              JSON.stringify(commandAliasData);
            errorMessage = await colorizer.colorizeMessageSimple(
              errorMessage,
              blackColorArray,
              true,
            );
        ...
        commandAliasEntity.toUpperCase() === commandAliasName.toUpperCase()
      ) {
        // Found a matching entry! 2
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.cFoundMatchingAliasEntry2,
        );
        commandAliasCount = commandAliasCount + 1;
        // Don't break, continue searching, so we get a full count of any duplicates found.
      }
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.ccommandAliasCountIs + JSON.stringify(commandAliasCount),
  );
  await loggers.consoleLog(namespacePrefix + functionName, msg.cEND_Function);
...
async function searchCommandAlias(commandAliasData, commandAliasName) {
  ...
  if (
    typeof commandAliasData === wrd.cobject &&
    typeof commandAliasName === wrd.cstring
  ) {
    for (let commandAliasEntity in commandAliasData) {
      // commandAliasEntity is:
      await loggers.consoleLog(
        ...
        if (commandAliasData[commandAliasEntity][wrd.cAliases] != undefined) {
          ...
          for (const element of arrayOfAliases) {
            ...
            if (
              ...
              commandAliasName.toUpperCase() ===
                bas.cDash + currentAlias.toUpperCase() ||
              commandAliasName.toUpperCase() ===
                bas.cDoubleDash + currentAlias.toUpperCase() ||
              commandAliasName.toUpperCase() ===
                bas.cForwardSlash + currentAlias.toUpperCase() ||
              commandAliasName.toUpperCase() ===
                bas.cBackSlash + currentAlias.toUpperCase() ||
              commandAliasName.toLowerCase() === currentAlias.toLowerCase() ||
              commandAliasName.toLowerCase() ===
                bas.cDash + currentAlias.toLowerCase() ||
              commandAliasName.toLowerCase() ===
                bas.cDoubleDash + currentAlias.toLowerCase() ||
              commandAliasName.toLowerCase() ===
                bas.cForwardSlash + currentAlias.toLowerCase() ||
              commandAliasName.toLowerCase() ===
                bas.cBackSlash + currentAlias.toLowerCase()
            ) {
          ...
          let commandAliasesObjectTemp = await searchCommandAlias(
            commandAliasData[commandAliasEntity],
            commandAliasName,
          );
          ...
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.ccommandAliasesObjectTempIs +
              JSON.stringify(commandAliasesObjectTemp),
          );
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.ccommandAliasObjectIs + JSON.stringify(commandAliasObject),
  );
  await loggers.consoleLog(namespacePrefix + functionName, msg.cEND_Function);
...
async function getAllCommandAliasData(commandAliasDataStructure) {
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.ccommandAliasDataStructureIs +
      JSON.stringify(commandAliasDataStructure),
  );
  let allCommandsData = false;
  let internalCommandAliasDataStructure;
  if (commandAliasDataStructure === undefined) {
    internalCommandAliasDataStructure = JSON.parse(
      JSON.stringify(D[sys.cCommandsAliases]),
    );
  } else {
    internalCommandAliasDataStructure = JSON.parse(
      JSON.stringify(commandAliasDataStructure),
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cinternalCommandAliasDataStructureIs +
      JSON.stringify(internalCommandAliasDataStructure),
  );
  if (typeof internalCommandAliasDataStructure === wrd.cobject) {
    ...
    for (let commandAliasEntity in internalCommandAliasDataStructure) {
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cinternalCommandAliasDataStructureCommandAliasEntityIs +
          JSON.stringify(internalCommandAliasDataStructure[commandAliasEntity]),
      );
      if (
        typeof internalCommandAliasDataStructure[commandAliasEntity] ===
        wrd.cobject
      ) {
        // internalCommandAliasDataStructure[commandAliasEntity] is of type object!
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.cgetAllCommandAliasDataMessage01,
        );
        let allCommandAliasesTemp;
        allCommandAliasesTemp = await getAllCommandAliasData(
          internalCommandAliasDataStructure[commandAliasEntity],
        );
        // allCommandAliasesTemp returned from the recursive call is:
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.callCommandAliasesTempReturnedFromRecursiveCallIs +
            JSON.stringify(allCommandAliasesTemp),
        );
        if (allCommandAliasesTemp === false) {
          // The recursive call returned false, so push the current entity to the output array!
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cgetAllCommandAliasDataMessage02,
          );
          allCommandsData.push(internalCommandAliasDataStructure);
          // allCommandsData after pushing to the array is:
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.callCommandsDataAfterPushingToTheArrayIs +
              JSON.stringify(allCommandsData),
          );
          break;
        } else {
          allCommandsData = await ruleBroker.processRules(
            [allCommandsData, allCommandAliasesTemp],
            [biz.cobjectDeepMerge],
          );
        }
      } else {
        if (
          Array.isArray(allCommandsData) === true &&
          allCommandsData.length === 0
        ) {
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.callCommandsDataIs + JSON.stringify(allCommandsData),
  );
  await loggers.consoleLog(namespacePrefix + functionName, msg.cEND_Function);
 ...
 * @date 2022/05/27
 */
async function getCommandNamespaceDataObject(
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cnamespaceToFindIs + namespaceToFind,
  );
  ...
  for (let commandAliasEntity in commandAliasDataStructure) {
    ...
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.ccommandAliasDataStructureCommandAliasEntityIs +
        JSON.stringify(commandAliasDataStructure[commandAliasEntity]),
    );
    if (commandAliasEntity === namespaceToFind) {
      namespaceCommandsObject = commandAliasDataStructure[commandAliasEntity];
      break;
    } else if (
      typeof commandAliasDataStructure[commandAliasEntity] === wrd.cobject
    ) {
      // Search recursively
      let namespaceCommandsTempObject = await getCommandNamespaceDataObject(
        commandAliasDataStructure[commandAliasEntity],
        namespaceToFind,
      );
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cnamespaceCommandsObjectIs + JSON.stringify(namespaceCommandsObject),
  );
  await loggers.consoleLog(namespacePrefix + functionName, msg.cEND_Function);
...
async function getCommandArgs(commandString, commandDelimiter) {
  ...
  let secondaryCommandArgsDelimiter =
    await configurator.getConfigurationSetting(
      wrd.csystem,
      cfg.csecondaryCommandDelimiter,
    );
  if (
    commandDelimiter === null ||
    commandDelimiter !== commandDelimiter ||
  ...
  if (
    ...
    if (commandString.includes(bas.cBackTickQuote) === true) {
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.ccommandStringContainsEitherSingleQuoteOrBackTickQuote,
      );
      ...
      if (commandString.includes(bas.cBackTickQuote) === true) {
        ...
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.ccommandStringContainsSingleQuote,
        );
        ...
        let numberOfSingleQuotes =
          commandString.split(bas.cBackTickQuote).length - 1;
        ...
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.cgetCommandArgsMessage1 + sys.cgetCommandArgsMessage2,
        );
        if (
          numberOfSingleQuotes >= 2 &&
          ...
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cnumberOfSingleQuotesIsEven,
          );
          ...
          for (let i = 0; i < numberOfSingleQuotes; i++) {
            ...
            if (i === 0) {
              ...
              indexOfStringDelimiter = commandString.indexOf(
                bas.cBackTickQuote,
                0,
              );
              // First index is:
              await loggers.consoleLog(
                namespacePrefix + functionName,
                msg.cFirstIndexIs + indexOfStringDelimiter,
              );
              ...
              commandString = await ruleBroker.processRules(
                [
                  commandString,
                  [indexOfStringDelimiter, bas.cBackTickQuote + bas.cTilde],
                ],
                replaceCharacterAtIndexRule,
              );
              stringLiteralCommandDelimiterAdded = true;
              // commandString after tagging the first string delimiter:
              await loggers.consoleLog(
                namespacePrefix + functionName,
                msg.ccommandStringAfterTaggingTheFirstStringDelimiter +
                  commandString,
              );
            } else {
              indexOfStringDelimiter = commandString.indexOf(
                bas.cBackTickQuote,
                indexOfStringDelimiter + 1,
              );
              // Additional index is:
              await loggers.consoleLog(
                namespacePrefix + functionName,
                msg.cAdditionalIndexIs + indexOfStringDelimiter,
              );
              ...
              if (
                ...
                commandString = await ruleBroker.processRules(
                  [
                    commandString,
                    [indexOfStringDelimiter, bas.cTilde + bas.cBackTickQuote],
                  ],
                  replaceCharacterAtIndexRule,
                );
                stringLiteralCommandDelimiterAdded = true;
                ...
                await loggers.consoleLog(
                  namespacePrefix + functionName,
                  msg.ccommandStringAfterTaggingAnOddStringDelimiter +
                    commandString,
                );
              } else {
                ...
                await loggers.consoleLog(
                  namespacePrefix + functionName,
                  msg.ccommandStringAfterTaggingAnEvenStringDelimiter +
                    commandString,
                );
              }
            }
          ...
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cpreSplitCommandStringIs +
              JSON.stringify(preSplitCommandString),
          );
          for (let j = 0; j < preSplitCommandString.length; j++) {
            ...
            if (preSplitCommandStringElement.includes(bas.cTilde) === false) {
              postSplitCommandString =
                preSplitCommandStringElement.split(commandArgsDelimiter);
              for (const element of postSplitCommandString) {
                if (element !== "") {
                  // postSplitCommandString[k] is:
                  await loggers.consoleLog(
                    namespacePrefix + functionName,
                    msg.cpostSplitCommandStringIs + JSON.stringify(element),
                  );
              ...
              if (
                returnData[returnData.length - 1].slice(-1) ===
                secondaryCommandArgsDelimiter
              ) {
                preSplitCommandStringElement = await ruleBroker.processRules(
                  [preSplitCommandStringElement, [/~/g, bas.cBackTickQuote]],
                  replaceTildesWithSingleQuoteRule,
                );
                returnData[returnData.length - 1] =
                  returnData[returnData.length - 1] +
                  preSplitCommandStringElement;
              } else {
                // preSplitCommandSringElement is:
                await loggers.consoleLog(
                  namespacePrefix + functionName,
                  msg.cpreSplitCommandStringElementIs +
                    JSON.stringify(preSplitCommandStringElement),
                );
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cDoingStraightSplitCommandString + commandString,
      );
  ...
  if (stringLiteralCommandDelimiterAdded === true) {
    ...
    await ruleBroker.processRules(
      [returnData, ""],
      [biz.cremoveStringLiteralTagsFromArray],
    );
  }
  await loggers.consoleLog(
    namespacePrefix + functionName,
...
async function executeCommand(commandString) {
  ...
  let returnData = [];
  let commandToExecute = await getValidCommand(
    commandString,
    await configurator.getConfigurationSetting(
      wrd.csystem,
      cfg.cprimaryCommandDelimiter,
    ),
  );
  // commandToExecute is:
  await loggers.consoleLog(
  ...
  let commandArgs = await getCommandArgs(
    commandString,
    await configurator.getConfigurationSetting(
      wrd.csystem,
      cfg.cprimaryCommandDelimiter,
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.ccommandArgsIs + commandArgs,
  );
  let commandMetricsEnabled = await configurator.getConfigurationSetting(
    wrd.csystem,
    cfg.cenableCommandPerformanceMetrics,
  );
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.ccommandQueueIs + (await queue.queuePrint(sys.cCommandQueue)),
  );
  ...
  if (commandMetricsEnabled === true) {
    ...
    commandStartTime = await ruleBroker.processRules(
      [gen.cYYYYMMDD_HHmmss_SSS, ""],
      [biz.cgetNowMoment],
    );
    // Command Start time is:
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cCommandStartTimeIs + commandStartTime,
    );
  ...
  if (
    commandMetricsEnabled === true &&
    commandToExecute !== "" &&
    commandToExecute !== false
  ) {
    let performanceTrackingObject = {};
    commandEndTime = await ruleBroker.processRules(
      [gen.cYYYYMMDD_HHmmss_SSS, ""],
      [biz.cgetNowMoment],
    );
    // Command End time is:
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cCommandEndTimeIs + commandEndTime,
    );
    // Now compute the delta tme so we know how long it took to run that command.
    commandDeltaTime = await ruleBroker.processRules(
      [commandStartTime, commandEndTime],
      [biz.ccomputeDeltaTime],
    );
    // Command run-time is:
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cCommandRunTimeIs + commandDeltaTime,
    );
    ...
    if (
      (await stack.contains(
        cfg.ccommandNamesPerformanceTrackingStack,
        commandToExecute,
      )) === false
    ) {
      await stack.push(
    ...
    await stack.push(
      cfg.ccommandsPerformanceTrackingStack,
      performanceTrackingObject,
    );
...
async function removePluginCommands(pluginName) {
  ...
  if (pluginConstantsValidation) {
    pluginConstantsValidationCommands =
      pluginConstantsValidation[sys.cpluginCommandConstantsValidation];
  } else {
  ...
  if (pluginConstantsValidationCommands) {
    try {
      for (const pluginCommandsRuleKey in pluginConstantsValidationCommands) {
        let pluginCommandConstValidationObject =
          pluginConstantsValidationCommands[pluginCommandsRuleKey];
        // pluginCommandConstValidationObject is:
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.cpluginCommandsConstValidationObjectIs +
            JSON.stringify(pluginCommandConstValidationObject),
        );
        // Removing plugin command:
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.cremovePluginCommandsMessage01 +
            pluginCommandConstValidationObject[wrd.cActual],
        );
