...
async function searchForPatternsInStringArray(inputData, inputMetaData) {
  ...
  if (inputData && inputData.length > 0) {
    ...
    for (let a = 0; a < inputData.length; a++) { // Initial high-level loop over each of the array elements. (This is the source string for the comparison)
      ...
      if (currentMasterStringArrayElement.includes(bas.cSpace) === false) {
        ...
        for (let b = minStringLength; b <= maxStringLength; b++) { // b will now hold the length of the string we are using to compare.
          ...
          loggers.consoleLog(namespacePrefix + functionName, msg.cSearchForPatternsInStringArrayMessage2 + b);
          // length of currentMasterStrngArrayElement is:
          loggers.consoleLog(namespacePrefix + functionName, msg.cSearchForPatternsInStringArrayMessage3 + currentMasterStringArrayElement.length);
          ...
          for (let c = 0; c <= currentMasterStringArrayElement.length - minStringLength; c++) { // Loop through each set of strings in the master comparison string.
            // c value is:
            loggers.consoleLog(namespacePrefix + functionName, msg.ccValueIs + c);
            ...
            // beginningIndex is:
            loggers.consoleLog(namespacePrefix + functionName, msg.cbeginningIndexIs + beginningIndex);
            let endingIndex = c + b;
            // endingIndex is:
            loggers.consoleLog(namespacePrefix + functionName, msg.cendingIndexIs + endingIndex);
            let stringToCompare = currentMasterStringArrayElement.substring(beginningIndex, endingIndex);
            // stringToCompare is:
            loggers.consoleLog(namespacePrefix + functionName, msg.cstringToCompareIs + stringToCompare);
            ...
            for (let d = 0; d < inputData.length; d++) {
              // d value is:
              loggers.consoleLog(namespacePrefix + functionName, msg.cdValueIs + d);
              if (d != a) {
                // d != a
                loggers.consoleLog(namespacePrefix + functionName, msg.cdNotEqualA);
                let otherStringToCompare = inputData[d];
                // otherStringToCompare is:
                loggers.consoleLog(namespacePrefix + functionName, msg.cotherStringToCompareIs + otherStringToCompare);
                if (otherStringToCompare.includes(stringToCompare)) {
                  // FOUND A MATCH!!!!
                  loggers.consoleLog(namespacePrefix + functionName, msg.cFoundAMatch + stringToCompare);
                  ...
                  if (await ruleParsing.processRulesInternal([[returnData, stringToCompare], ''], [biz.cdoesArrayContainValue]) === false) {
                    ...
                    loggers.consoleLog(namespacePrefix + functionName, msg.cSearchForPatternsInStringArrayMessage4 + stringToCompare);
        ...
        loggers.consoleLog(namespacePrefix + functionName, msg.cSearchForPatternsInSringArrayMessage5 + msg.cSearchForPatternsInStringArrayMessage6);
      }
    ...
    loggers.consoleLog(namespacePrefix + functionName, msg.cSearchForPatternsInSringArrayMessage7);
  }
