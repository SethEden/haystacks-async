 ...
 * @return {string|boolean} The delimited list of possible combinations generated by solving the Lehmer Code, or false if error is encountered.
 * @author Seth Hollingsead
 * @date 2022/01/20
...
async function solveLehmerCode(inputData, inputMetaData) {
  ...
  if (Array.isArray(inputData) && inputData.every(item => Number.isInteger(item))) {
    if (Array.isArray(inputMetaData) && inputMetaData.every(item => Array.isArray(item) && item.every(subItem => typeof subItem === wrd.cstring))) {
      returnData = ''
 ...
 * @return {array<array<integer>>|boolean} The final list of arrays after the array expansion has completed successfully, or false if error is encountered.
 * @author Seth Hollingsead
 * @date 2022/01/20
 */
async function recursiveArrayExpansion(inputData, inputMetaData) {
  ...
  if (inputData && inputDataIsArray === true && inputData.length > 0) {
    if (inputMetaData && inputMetaDataIsArray === true && inputMetaData.length > 0) {
      returnData = [];
 ...
 * @return {string|boolean} The joined string from each of the array element strings at the Lehmer code indices, or false if error is encountered.
 * @author Seth Hollingsead
 * @date 2022/01/20
 */
 ...
 * @return {string|boolean} A coma-separated list of every possible combination of command aliases, or false if error is encountered.
 * @author Seth Hollingsead
 * @date 2022/01/21
 */
 ...
 * @param {string} inputMetaData Not used for this business rule.
 * @return {string|boolean} A single string command line of code that should be sent to the command parser, or false if error is encountered.
 * @author Seth Hollingsead
 * @date 2022/01/21
 */
