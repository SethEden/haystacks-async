...
async function solveLehmerCode(inputData, inputMetaData) {
  ...
  if (inputData) {
    ...
    // expandedArray is:
    loggers.consoleLog(namespacePrefix + functionName, msg.cexpandedArrayIs + JSON.stringify(expandedArray));
...
async function recursiveArrayExpansion(inputData, inputMetaData) {
  ...
  if (inputData && inputMetaData && inputDataIsArray === true && inputMetaDataIsArray === true && inputData.length > 0 && inputMetaData.length > 0) {
    ...
    if (indexOfExpansion < arrayToBeExpanded.length - 1) {
      ...
      // returnDataTemp is:
      loggers.consoleLog(namespacePrefix + functionName, msg.creturnDataTempIs + JSON.stringify(returnDataTemp));
      ...
      for (let arrayElement in returnDataTemp) {
        // arrayElement is:
        loggers.consoleLog(namespacePrefix + functionName, msg.carrayElementIs + JSON.stringify(arrayElement));
        ...
        // tempReturnData1 is:
        loggers.consoleLog(namespacePrefix + functionName, msg.ctempReturnData1Is + JSON.stringify(await tempReturnData1));
        ...
        for (let k = 0; k <= tempReturnData1.length - 1; k++) {
          ...
          if (await ruleParsing.processRulesInternal([[masterTempReturnData, tempReturnData1[k]], ''], [biz.cdoesArrayContainValue]) === false) {
            ...
            loggers.consoleLog(namespacePrefix + functionName, msg.cpushingTempReturnData1Kvalue + JSON.stringify(await tempReturnData1[k]));
  ...
  loggers.consoleLog(namespacePrefix + functionName, msg.creturnDataIs + JSON.stringify(await returnData));
  loggers.consoleLog(namespacePrefix + functionName, msg.cEND_Function);
