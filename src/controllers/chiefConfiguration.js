...
async function setupConfiguration(appConfigPath, frameworkConfigPath) {
  ...
  await loggers.consoleLog(
    ...
    msg.cappConfigPathIs + appConfigPath,
  );
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cframeworkConfigPathIs + frameworkConfigPath,
  );
  ...
  frameworkConfigPath = await ruleBroker.processRules(
    [frameworkConfigPath, ""],
    rules,
  );
  ...
  allFrameworkConfigData = await chiefData.setupAllJsonConfigData(
    sys.cframeworkConfigPath,
    wrd.cconfiguration,
  );
  allAppConfigData = await chiefData.setupAllJsonConfigData(
    sys.cappConfigPath,
    wrd.cconfiguration,
  );
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cALL_DATA_IS + JSON.stringify(D),
  );
...
async function setupPluginConfiguration(pluginConfigPath) {
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cpluginConfigPathIs + pluginConfigPath,
  );
  let rules = [biz.cswapBackSlashToForwardSlash];
  pluginConfigPath = await ruleBroker.processRules(
    [pluginConfigPath, ""],
    rules,
  );
  // pluginConfigPath is:
  ...
  allPluginConfigData = await chiefData.setupAllJsonConfigPluginData(
    pluginConfigPath,
    wrd.cconfiguration,
  );
  let allParsedPluginConfigData =
    await parsePluginConfigurationData(allPluginConfigData);
  // allParsedPluginConfigData is:
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.callParsedPluginConfigDataIs +
      JSON.stringify(allParsedPluginConfigData),
  );
  await loggers.consoleLog(namespacePrefix + functionName, msg.cEND_Function);
...
async function parsePluginConfigurationData(allPluginConfigData) {
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.callPluginConfigDataIs + JSON.stringify(allPluginConfigData),
  );
  ...
  highLevelPluginSystemConfigurationContainer =
    allPluginConfigData[wrd.csystem];
  // highLevelPluginSystemConfigurationContainer is:
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.chighLevelPluginSystemConfigurationContainerIs +
      JSON.stringify(highLevelPluginSystemConfigurationContainer),
  );
  highLevelPluginDebugConfigurationContainer =
    allPluginConfigData[cfg.cdebugSettings];
  // highLevelPluginDebugConfigurationContainer is:
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.chighLevelPluginDebugConfigurationContainerIs +
      JSON.stringify(highLevelPluginDebugConfigurationContainer),
  );
  ...
  if (highLevelPluginSystemConfigurationContainer) {
    ...
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cbeginProcessingHighLevelPluginSystemConfigurationContainer,
    );
    for (let key in highLevelPluginSystemConfigurationContainer) {
      ...
      if (!!value || value === false) {
        ...
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.cfullyQualifiedNameIs + fullyQualifiedName,
        );

        name =
          await configurator.processConfigurationNameRules(fullyQualifiedName);
        // name is:
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.cnameIs + name,
        ...
        let tempReturnData1 = await configurator.setPluginConfigurationSetting(
          returnData,
          namespace,
          name,
          value,
        ...
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.ctempReturnData1Is + JSON.stringify(tempReturnData1),
        );
        // returnData[wrd.csystem] = Object.assign(returnData[wrd.csystem], tempReturnData1);
        // returnData after object.assign is:
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.creturnDataAfterObjectAssignIs + JSON.stringify(returnData),
        );
    ...
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cendProcessingHighLevelPluginSystemConfigurationContainer,
    );
  ...
  if (highLevelPluginDebugConfigurationContainer) {
    ...
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cbeginProcessingHighLevelPluginDebugConfigurationContainer,
    );
    ...
    for (let key in highLevelPluginDebugConfigurationContainer) {
      ...
      if (!!value || value === false) {
        ...
        let tempReturnData2 = await configurator.setPluginConfigurationSetting(
          returnData,
          namespace,
          name,
          value,
        ...
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.ctempReturnData2Is + JSON.stringify(tempReturnData2),
        );
    ...
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cendProcessingHighLevelPluginDebugConfigurationContainer,
    );
...
async function parseLoadedConfigurationData(allConfigurationData) {
  ...
  highLevelDebugConfigurationContainer =
    allConfigurationData[cfg.cdebugSettings];
  ...
  if (highLevelSystemConfigurationContainer) {
    for (let key in highLevelSystemConfigurationContainer) {
      ...
      if (!!value || value === false) {
        ...
        if (
          namespace === wrd.csystem &&
          name === cfg.cdebugSettings &&
