...
async function version(inputData, inputMetaData) {
  ...
  if (inputData.length === 2) {
    ...
    if (appContext.toUpperCase() === wrd.cAPPLICATION) {
      configVersion = await configurator.getConfigurationSetting(
        wrd.csystem,
...
async function about(inputData, inputMetaData) {
  ...
  if (inputData.length === 2) {
    appContext = inputData[1];
    if (appContext.toUpperCase() === wrd.cAPPLICATION) {
      configDescription = await configurator.getConfigurationSetting(
        wrd.csystem,
...
async function name(inputData, inputMetaData) {
  ...
  if (inputData.length === 3) {
    appContext = inputData[1];
    useFancyFont = await ruleBroker.processRules(
      [inputData[2], ""],
  ...
  if (appContext !== "") {
    if (appContext.toUpperCase() === wrd.cAPPLICATION) {
      reportedName = await configurator.getConfigurationSetting(
        wrd.csystem,
  ...
  if (useFancyFont === true) {
    figletFont = await configurator.getConfigurationSetting(
      wrd.csystem,
      cfg.cfigletFont,
    );
    console.log(
      figlet.textSync(reportedName, {
        font: figletFont,
        horizontalLayout: wrd.cfull,
      }),
    );
  } else {
...
async function help(inputData, inputMetaData) {
  ...
  let validCommandNamespaceTypes = [
    wrd.cFramework,
    wrd.cApplication,
    wrd.cPlugins,
  ];
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.ccommandNamespaceTypesInputArrayIs +
      JSON.stringify(commandNamespaceTypesInputArray),
  );
  ...
  if (commandNamespaceTypesInputArray.length > 0) {
    for (let commandNamespaceEntityKey in commandNamespaceTypesInputArray) {
      let commandNamespaceEntity =
        commandNamespaceTypesInputArray[commandNamespaceEntityKey];
      if (
        commandNamespaceEntity.toUpperCase().trim() === wrd.cFRAMEWORK ||
        commandNamespaceEntity.toUpperCase().trim() === wrd.cPLATFORM
      ) {
        commandNamespaceTypesConfirmedArray.push(wrd.cFramework);
      } else if (
        commandNamespaceEntity.toUpperCase().trim() === wrd.cAPPLICATION ||
        commandNamespaceEntity.toUpperCase().trim() === wrd.cAPP
      ) {
        commandNamespaceTypesConfirmedArray.push(wrd.cApplication);
      } else if (
        commandNamespaceEntity.toUpperCase().trim() === wrd.cPLUGINS ||
        commandNamespaceEntity.toUpperCase().trim() === wrd.cPLUGIN
      ) {
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.ccommandNamespaceTypesConfirmedArrayIs +
      JSON.stringify(commandNamespaceTypesConfirmedArray),
  );

  if (validUserEntry) {
    if (commandNamespaceTypesConfirmedArray.length > 0) {
      for (const commandNamespaceTypeConfirmedKey in commandNamespaceTypesConfirmedArray) {
        const commandNamespaceTypeConfirmedEntity =
          commandNamespaceTypesConfirmedArray[commandNamespaceTypeConfirmedKey];
        if (commandNamespaceTypeConfirmedEntity === wrd.cFramework) {
          // processing framework commands
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cprocessingFrameworkCommands,
          );
          let frameworkCommandAliases =
          ...
          if (frameworkCommandAliases) {
            if (Object.keys(namespaceAllCommandsDataObject).length != 0) {
              ...
              namespaceAllCommandsDataObject = await ruleBroker.processRules(
          ...
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cprocessingApplicationCommands,
          );
          ...
          if (
            ...
            await loggers.consoleLog(
              namespacePrefix + functionName,
              msg.cprocessingPluginsCommands,
            );
            let pluginsCommandAliases =
              await commandBroker.getAllCommandAliasData(
          ...
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cprocessingCommandsFor + commandNamespaceTypeConfirmedEntity,
          );
          let namespaceCommandsData =
            await commandBroker.getCommandNamespaceDataObject(
              undefined,
              commandNamespaceTypeConfirmedEntity,
            );
          // namespaceCommandsData is:
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cnamespaceCommandsDataIs +
              JSON.stringify(namespaceCommandsData),
          );
          if (namespaceCommandsData === false) {
            ...
            errorMessage =
              msg.chelpCommandMessage01 +
              bas.cSpace +
              msg.chelpCommandMessage02;
            console.log(errorMessage);
          } else {
            // NOW call getAllCommandAliasData with the above found data!
            await loggers.consoleLog(
              namespacePrefix + functionName,
              msg.chelpCommandMessage03,
            );
            let extraUserEnteredCommandAliases =
              await commandBroker.getAllCommandAliasData(namespaceCommandsData);
            if (extraUserEnteredCommandAliases) {
              if (Object.keys(namespaceAllCommandsDataObject).length != 0) {
                // extraUserEnteredCommandAliases is:
                await loggers.consoleLog(
                  namespacePrefix + functionName,
                  msg.cextraUserEnteredCommandAliasesIs +
                    JSON.stringify(extraUserEnteredCommandAliases),
                );
                namespaceAllCommandsDataObject = await ruleBroker.processRules(
                  [
                    namespaceAllCommandsDataObject,
                    extraUserEnteredCommandAliases,
                  ],
                  [biz.cobjectDeepMerge],
                );
              } else {
        ...
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.cnamespaceAllCommandsDataObjectIs +
            JSON.stringify(namespaceAllCommandsDataObject),
        );
      ...
      namespaceAllCommandsDataObject =
        await commandBroker.getAllCommandAliasData(D[sys.cCommandsAliases]);
    }
    returnData[1] = await ruleBroker.processRules(
      [namespaceAllCommandsDataObject, ""],
      [biz.carrayDeepClone],
    );
    // namespaceAllCommandsDataObject is:
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cnamespaceAllCommandsDataObjectIs +
        JSON.stringify(namespaceAllCommandsDataObject[0]),
    );
    await loggers.consoleTableLog(
      baseFileName + bas.cDot + functionName,
      namespaceAllCommandsDataObject[0],
      [wrd.cName, wrd.cDescription],
    );
  }
  await loggers.consoleLog(
    namespacePrefix + functionName,
...
async function workflowHelp(inputData, inputMetaData) {
  ...
  let validWorkflowsNamespaceTypes = [
    wrd.cFramework,
    wrd.cApplication,
    wrd.cPlugins,
  ];
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cworkflowNamespaceTypesInputArrayIs +
      JSON.stringify(workflowNamespaceTypesInputArray),
  );
  ...
  if (workflowNamespaceTypesInputArray.length > 0) {
    for (let workflowNamespaceEntityKey in workflowNamespaceTypesInputArray) {
      let workflowNamespaceEntity =
        workflowNamespaceTypesInputArray[workflowNamespaceEntityKey];
      if (
        workflowNamespaceEntity.toUpperCase().trim() === wrd.cFRAMEWORK ||
        workflowNamespaceEntity.toUpperCase().trim() === wrd.cPLATFORM
      ) {
        workflowNamespaceTypesConfirmedArray.push(wrd.cFramework);
      } else if (
        workflowNamespaceEntity.toUpperCase().trim() === wrd.cAPPLICATION ||
        workflowNamespaceEntity.toUpperCase().trim() === wrd.cAPP
      ) {
        workflowNamespaceTypesConfirmedArray.push(wrd.cApplication);
      } else if (
        workflowNamespaceEntity.toUpperCase().trim() === wrd.cPLUGINS ||
        workflowNamespaceEntity.toUpperCase().trim() === wrd.cPLUGIN
      ) {
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.cworkflowNamespaceTypesConfirmedArrayIs +
      JSON.stringify(workflowNamespaceTypesConfirmedArray),
  );

  if (validUserEntry) {
    if (workflowNamespaceTypesConfirmedArray.length > 0) {
      for (const workflowNamespaceTypeConfirmedKey in workflowNamespaceTypesConfirmedArray) {
        const workflowNamespaceTypeConfirmedEntity =
          workflowNamespaceTypesConfirmedArray[
            workflowNamespaceTypeConfirmedKey
          ];
        if (workflowNamespaceTypeConfirmedEntity === wrd.cFramework) {
          // processing framework workflows
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cprocessingFrameworkWorkflows,
          );
          let frameworkWorkflows = await workflowBroker.getAllWorkflows(
          ...
          if (frameworkWorkflows) {
            if (namespaceAllWorkflowsDataObject.length != 0) {
              ...
              await loggers.consoleLog(
                namespacePrefix + functionName,
                msg.cframeworkWorkflowsIs + JSON.stringify(frameworkWorkflows),
              );
          ...
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cprocessingApplicationWorkflows,
          );
          let applicationWorkflows = await workflowBroker.getAllWorkflows(
          ...
          if (applicationWorkflows) {
            if (namespaceAllWorkflowsDataObject.length != 0) {
              ...
              await loggers.consoleLog(
                namespacePrefix + functionName,
                msg.capplicationWorkflowsIs +
                  JSON.stringify(applicationWorkflows),
              );
          ...
          if (
            ...
            await loggers.consoleLog(
              namespacePrefix + functionName,
              msg.cprocessingPluginsWorkflows,
            );
            let pluginWorkflows = await workflowBroker.getAllWorkflows(
            ...
            if (pluginWorkflows) {
              if (namespaceAllWorkflowsDataObject.length != 0) {
                ...
                await loggers.consoleLog(
                  namespacePrefix + functionName,
                  msg.cpluginWorkflowsIs + JSON.stringify(pluginWorkflows),
                );
          ...
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cprocessingWorkflowsFor + workflowNamespaceTypeConfirmedEntity,
          );
          let namespaceWorkflowData =
            await workflowBroker.getWorkflowNamespaceDataObject(
              undefined,
              workflowNamespaceTypeConfirmedEntity,
            );
          // namespaceWorkflowData is:
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cnamespaceWorkflowDataIs +
          ...
          if (namespaceWorkflowData === false) {
            ...
            // Please make sure you have entered the correct name and try again.
            errorMessage =
              msg.cworkflowHelpCommandMessage01 +
              bas.cSpace +
              msg.chelpCommandMessage02;
            console.log(errorMessage);
          } else {
            ...
            await loggers.consoleLog(
              namespacePrefix + functionName,
              msg.cworkfowHelpMessage03,
            );
            let extraUserEnteredWorkflows =
              await workflowBroker.getAllWorkflows(namespaceWorkflowData);
            if (extraUserEnteredWorkflows) {
              if (namespaceAllWorkflowsDataObject.length != 0) {
                // extraUserEnteredWorkflows is:
                await loggers.consoleLog(
                  namespacePrefix + functionName,
                  msg.cextraUserEnteredWorkflowsIs +
                    JSON.stringify(extraUserEnteredWorkflows),
                );
                ...
                namespaceAllWorkflowsDataObject.push(
                  ...extraUserEnteredWorkflows,
                );
              } else {
        ...
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.cnamespaceAllWorkflowsDataObjectIs +
            JSON.stringify(namespaceAllWorkflowsDataObject),
        );
...
async function printUserCommandsLog(inputData, inputMetaData) {
  ...
  if (
    ...
    if ((await stack.isEmpty(sys.cUserEnteredCommandLog)) === false) {
      let userEnteredCommandLogArray = await stack.getStackContents(
        sys.cUserEnteredCommandLog,
      );
      let userEnteredCommandLog = userEnteredCommandLogArray.join(
        bas.cComa + bas.cNewLine + bas.cCarriageReturn,
      );
    ...
    console.log(
      msg.cprintUserCommandLogMessage01 +
        bas.cSpace +
        msg.cprintUserCommandLogMessage02,
    );
    returnData[1] = false;
  }
  await loggers.consoleLog(
...
async function printAllCommandsLog(inputData, inputMetaData) {
  ...
  if (
    ...
    if ((await stack.isEmpty(sys.cSystemCommandLog)) === false) {
      let allCommandsLogArray = await stack.getStackContents(
        sys.cSystemCommandLog,
      );
      let allCommandsLog = allCommandsLogArray.join(
        bas.cComa + bas.cNewLine + bas.cCarriageReturn,
      );
    ...
    console.log(
      msg.cprintAllCommandLogMessage01 +
        bas.cSpace +
        msg.cprintAllCommandLogMessage02,
    );
    returnData[1] = false;
  }
  await loggers.consoleLog(
...
async function clearUserCommandsLog(inputData, inputMetaData) {
  ...
  if (
    ...
    if (
      (await configurator.getConfigurationSetting(
        wrd.csystem,
        cfg.cenableUserCommandsLogClearing,
      )) === true
    ) {
...
async function clearAllCommandsLog(inputData, inputMetaData) {
  ...
  if (
    ...
    if (
      (await configurator.getConfigurationSetting(
        wrd.csystem,
        cfg.cenableAllCommandsLogClearing,
      )) === true
    ) {
