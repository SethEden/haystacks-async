...
async function saveConfiguration(inputData, inputMetaData) {
  ...
  let returnData = [true, {}];
  returnData[1] = dataBroker.writeJsonDataToFile(await configurator.getConfigurationSetting(wrd.csystem, cfg.cappConfigPath) + wrd.cconfig + gen.cDotjson, JSON.stringify(D));
...
async function changeDebugConfigurationTheme(inputData, inputMetaData) {
  ...
  if (inputData && inputData.length === 2) {
    ...
    // frameworkThemesPath is:
    loggers.consoleLog(namespacePrefix + functionName, msg.cframeworkThemesPathIs + frameworkThemesPath);
    let namedThemePath = themeBroker.getNamedThemePathFromRootPath(desiredThemeName, frameworkThemesPath);
