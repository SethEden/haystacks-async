...
async function validateConstants(inputData, inputMetaData) {
  ...
  if (
    ...
    let validationFrameworkArray =
      D[sys.cConstantsValidationData][wrd.cFramework][sys.cConstantsFilePaths]; // Framework constants paths
    let validationApplicationArray =
      D[sys.cConstantsValidationData][wrd.cApplication][
        sys.cConstantsFilePaths
      ]; // Application constants paths
    let validationPluginsMetaArray =
      D[sys.cConstantsValidationData][wrd.cPlugins]; // Will need to iterate through each of the plugins and capture the plugin constants path for each plugin!
    let phase1FinalResult = true;
    let phase2FinalResult = true;
    let phase1Results = {};
    let phase2Results = {};
    let validConstantsValidationTypes = [
      wrd.cFramework,
      wrd.cApplication,
      wrd.cPlugins,
    ];
    let validConstantsValidationUserTypes = [
      wrd.cFramework,
      wrd.cPlatform,
      wrd.cApplication,
      wrd.cApp,
    ...
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cvalidationFrameworkArrayIs +
        JSON.stringify(validationFrameworkArray),
    );
    // validationApplicationArray is:
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cvalidationApplicationArrayIs +
        JSON.stringify(validationApplicationArray),
    );
    // validationPluginsMetaArray is:
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cvalidationPluginsMetaArrayIs +
        JSON.stringify(validationPluginsMetaArray),
    );
    ...
    if (Array.isArray(inputData) && inputData.length === 2) {
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cinputDataLengthIs + inputData.length,
      );
    ...
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cvalidationTypesInputArrayIs +
        JSON.stringify(validationTypesInputArray),
    );
    ...
    if (validationTypesInputArray.length > 0) {
      for (let validationTypesKey in validationTypesInputArray) {
        let userEnteredValidationType =
          validationTypesInputArray[validationTypesKey];
        if (
          userEnteredValidationType.toUpperCase().trim() === wrd.cFRAMEWORK ||
          userEnteredValidationType.toUpperCase().trim() === wrd.cPLATFORM
        ) {
          validationTypesConfirmedArray.push(wrd.cFramework);
        } else if (
          userEnteredValidationType.toUpperCase().trim() === wrd.cAPPLICATION ||
          userEnteredValidationType.toUpperCase().trim() === wrd.cAPP
        ) {
          validationTypesConfirmedArray.push(wrd.cApplication);
        } else if (
          userEnteredValidationType.toUpperCase().trim() === wrd.cPLUGINS ||
          userEnteredValidationType.toUpperCase().trim() === wrd.cPLUGIN
        ) {
          ...
          console.log(
            msg.cWarningUserEnteredConstantsValidationDataTypeMessage01 +
              userEnteredValidationType,
          );
          console.log(
            msg.cWarningUserEnteredConstantsValidationDataTypeMessage02 +
              validConstantsValidationUserTypes.join(bas.cComa + bas.cSpace),
          );
        }
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cvalidationTypesConfirmedArrayIs +
          JSON.stringify(validationTypesConfirmedArray),
      );

      if (validationTypesConfirmedArray.length > 0) {
        validationTypesConfirmedList = validationTypesConfirmedArray.join(
          bas.cComa,
        );
        validUserEntry = true;
      } else {
        ...
        console.log(
          msg.cWarningUserEnteredConstantsValidationDataTypeMessage03,
        );
        // Constants validation types are:
        console.log(
          msg.cWarningUserEnteredConstantsValidationDataTypeMessage02 +
      ...
      validationTypesConfirmedList = validConstantsValidationTypes.join(
        bas.cComa,
      );
      validUserEntry = true;
    }
    ...
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cvalidationTypesConfirmedListIs + validationTypesConfirmedList,
    );
    ...
    if (validUserEntry) {
      ...
      if (validationTypesConfirmedList.includes(wrd.cApplication)) {
        validationArray = Object.assign(
          validationArray,
          validationApplicationArray,
        );
      }
      // validationArray before plugin constants validation data merge is:
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cvalidationArrayBeforePluginConstantsValidationDataMergeIs +
          JSON.stringify(validationArray),
      );
      if (
        await configurator.getConfigurationSetting(
          wrd.csystem,
        ...
        if (validationTypesConfirmedList.includes(wrd.cPlugins)) {
          for (let plugin in validationPluginsMetaArray) {
            ...
            await loggers.consoleLog(
              namespacePrefix + functionName,
              msg.cpluginIs + plugin,
            );
            let pluginValidationPathData =
              validationPluginsMetaArray[plugin][sys.cConstantsFilePaths];
            for (let constantsFilePathName in pluginValidationPathData) {
              // constantsFilePathName is:
              await loggers.consoleLog(
                namespacePrefix + functionName,
                msg.cconstantsFilePathNamesIs + constantsFilePathName,
              );
              let constantsFilePathValue =
                pluginValidationPathData[constantsFilePathName];
              // constantsFilePathValue is:
              await loggers.consoleLog(
                namespacePrefix + functionName,
                msg.cconstantsFilePathValueIs + constantsFilePathValue,
              );
              let newPluginConstantValidationName =
                plugin + bas.cColon + constantsFilePathName;
              // newPluginConstantValidationName is:
              await loggers.consoleLog(
                namespacePrefix + functionName,
                msg.cnewPluginConstantValidationNameIs +
                  newPluginConstantValidationName,
              );
              let newPluginConstantValidationPathObject = {
                [newPluginConstantValidationName]: constantsFilePathValue,
              };
              validationArray = Object.assign(
                validationArray,
                newPluginConstantValidationPathObject,
              );
              // validationArray after plugin constants validation data merge is:
              await loggers.consoleLog(
                namespacePrefix + functionName,
                msg.cvalidationArrayAfterPluginConstantsValidationDataMergeIs +
                  JSON.stringify(validationArray),
              );
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cvalidationArrayIs + JSON.stringify(validationArray),
      );
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cBeginPhase1ConstantsValidation,
      );
      ...
      for (let key1 in validationArray) {
        ...
        phase1Results[key1] = await ruleBroker.processRules(
          [constantsPath, key1],
          [biz.cvalidateConstantsDataValidation],
        );
      } // End-for (let key1 in validationArray)
      // END Phase 1 Constants Validation
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cEndPhase1ConstantsValidation,
      );
      // phase1Results is:
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cphase1ResultsIs + JSON.stringify(phase1Results),
      );
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cBeginPhase2ConstantsValidation,
      );
      ...
      for (let key2 in validationArray) {
        phase2Results[key2] = await ruleBroker.processRules(
          [key2, ""],
          [biz.cvalidateConstantsDataValues],
        );
      } // End-for (let key2 in validationArray)
      // END Phase 2 Constants Validation
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cEndPhase2ConstantsValidation,
      );
      // phase2Results is:
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cphase2ResultsIs + JSON.stringify(phase2Results),
      );

      for (let key3 in phase1Results) {
        let constantsPhase1ValidationNamespaceParentObject =
          await ruleBroker.processRules(
            [key3, ""],
            [biz.cgetConstantsValidationNamespaceParentObject],
          );
        if (
          key3.includes(bas.cColon) &&
          key3.toUpperCase().includes(wrd.cPLUGIN)
        ) {
        ...
        if (processingPluginResults === false) {
          await loggers.constantsValidationSummaryLog(
            constantsPhase1ValidationNamespaceParentObject[
      ...
      for (let key4 in phase2Results) {
        let constantsPhase2ValidationNamespaceParentObject =
          await ruleBroker.processRules(
            [key4, ""],
            [biz.cgetConstantsValidationNamespaceParentObject],
          );
        if (
          key4.includes(bas.cColon) &&
          key4.toUpperCase().includes(wrd.cPLUGIN)
        ) {
        ...
        if (processingPluginResults === false) {
          await loggers.constantsValidationSummaryLog(
            constantsPhase2ValidationNamespaceParentObject[
    ...
    console.log(
      msg.ccconstantsGeneratorMessage3 + msg.cconstantsGeneratorMessage4,
    );
    await configurator.setConfigurationSetting(
      wrd.csystem,
      cfg.cpassAllConstantsValidation,
...
async function validateCommandAliases(inputData, inputMetaData) {
  ...
  let validCommandAliasesValidationTypes = [
    wrd.cFramework,
    wrd.cApplication,
    wrd.cPlugins,
  ];
  let validCommandAliasesValidationUserTypes = [
    wrd.cFramework,
    wrd.cPlatform,
    wrd.cApplication,
    wrd.cApp,
  ...
  if (validationTypesInputArray.length > 0) {
    for (let validationTypesKey in validationTypesInputArray) {
      ...
      if (
        ...
        console.log(
          msg.cWarningUserEnteredCommandAliasesValidationDataTypeMessage02 +
            validCommandAliasesValidationUserTypes.join(bas.cComa + bas.cSpace),
        );
      }
    } // End-for (let validationTypesKey in validationTypesInputArray)
    ...
    if (validationTypesConfirmedArray.length > 0) {
      ...
      console.log(
        msg.cWarningUserEnteredCommandAliasesValidationDataTypeMessage03,
      );
      // Command Aliases validation types are:
      console.log(
        msg.cWarningUserEnteredCommandAliasesValidationDataTypeMessage02 +
    ...
    // User didn't enter any parameters at all....just run it all!
    validationTypesConfirmedList = validCommandAliasesValidationTypes.join(
      bas.cComa,
    );
    validUserEntry = true;
  }
  ...
  if (validUserEntry === true) {
    if (validationTypesConfirmedList.includes(wrd.cFramework)) {
      let frameworkCommandAliases = await commandBroker.getAllCommandAliasData(
        D[sys.cCommandsAliases][wrd.cFramework],
      );
      // frameworkCommandAliases is:
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cframeworkCommandAliasesIs +
          JSON.stringify(frameworkCommandAliases),
      );
      if (frameworkCommandAliases) {
        allCommandAliasesToValidate = await ruleBroker.processRules(
          [allCommandAliasesToValidate, frameworkCommandAliases],
          [biz.cobjectDeepMerge],
        );
      }
    }
    if (validationTypesConfirmedList.includes(wrd.cApplication)) {
      let applicationCommandAliases =
        await commandBroker.getAllCommandAliasData(
          D[sys.cCommandsAliases][wrd.cApplication],
        );
      // applicationCommandAliases is:
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.capplicationCommandAliasesIs +
          JSON.stringify(applicationCommandAliases),
      );
    ...
    if (
      ...
      if (validationTypesConfirmedList.includes(wrd.cPlugins)) {
        let pluginCommandAliases = await commandBroker.getAllCommandAliasData(
          D[sys.cCommandsAliases][wrd.cPlugins],
        );
        // pluginCommandAliases is:
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.cpluginCommandAliasesIs + JSON.stringify(pluginCommandAliases),
        );
    ...
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.callCommandAliasesToValidateIs +
        JSON.stringify(allCommandAliasesToValidate),
    );
    ...
    for (let key1 in allCommandAliasesToValidate[0]) {
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.ccurrentCommandIs + JSON.stringify(currentCommand),
      );
      ...
      for (let j = 0; j < arrayOfAliases.length; j++) {
        ...
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.cBEGIN_jthLoop + j,
        );
        let currentAlias = arrayOfAliases[j];
        // currentAlias is:
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.ccurrentAliasIs + currentAlias,
        );
        duplicateAliasCount = await commandBroker.countMatchingCommandAlias(
          D[sys.cCommandsAliases],
          currentAlias,
        );
        if (duplicateAliasCount > 1) {
          // duplicateAliasCount is:
          let duplicateAliasCountMessage =
            msg.cduplicateAliasCountIs + duplicateAliasCount;
          duplicateAliasCountMessage = await colorizer.colorizeMessageSimple(
            duplicateAliasCountMessage,
            blackColorArray,
            true,
          );
          duplicateAliasCountMessage = await colorizer.colorizeMessageSimple(
          ...
          let duplicateAliasCommandMessage =
            msg.cduplicateCommandAliasIs + currentAlias;
          duplicateAliasCommandMessage = await colorizer.colorizeMessageSimple(
            duplicateAliasCommandMessage,
            blackColorArray,
            true,
          );
          duplicateAliasCommandMessage = await colorizer.colorizeMessageSimple(
...
async function validateWorkflows(inputData, inputMetaData) {
  ...
  let redColorArray = await colorizer.getNamedColorData(clr.cRed, [255, 0, 0]);
  let validWorkflowsValidationTypes = [
    wrd.cFramework,
    wrd.cApplication,
    wrd.cPlugins,
  ];
  let validWorkflowsValidationUserTypes = [
    wrd.cFramework,
    wrd.cPlatform,
    wrd.cApplication,
    wrd.cApp,
  ...
  if (validationTypesInputArray.length > 0) {
    for (let validationTypesKey in validationTypesInputArray) {
      ...
      if (
        ...
        console.log(
          msg.cWarningUserEnteredWorkflowsValidationDataTypeMessage02 +
            validWorkflowsValidationUserTypes.join(bas.cComa + bas.cSpace),
        );
      }
    } // End-for (let validationTypesKey in validationTypesInputArray)
    ...
    // User didn't enter any parameters at all....just run it all!
    validationTypesConfirmedList = validWorkflowsValidationTypes.join(
      bas.cComa,
    );
    validUserEntry = true;
  }
  ...
  if (validUserEntry === true) {
    if (validationTypesConfirmedList.includes(wrd.cFramework)) {
      allWorkflowsToValidate = await workflowBroker.getAllWorkflows(
        D[sys.cCommandWorkflows][wrd.cFramework],
      );
    }
    // allWorkflowsToValidate is: 1
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.callWorkflowsToValidate +
        num.c1 +
        bas.cSpace +
        JSON.stringify(allWorkflowsToValidate),
    );
    if (validationTypesConfirmedList.includes(wrd.cApplication)) {
      allWorkflowsToValidate.push(
        ...(await workflowBroker.getAllWorkflows(
          D[sys.cCommandWorkflows][wrd.cApplication],
        )),
      );
    }
    // If the system is either loading plugins or plugin data has been loaded by the build-release-deployment system for
    ...
    if (
      ...
      if (
        validationTypesConfirmedList.includes(wrd.cPlugins) &&
        D[sys.cCommandWorkflows][wrd.cPlugins] !== undefined
      ) {
        allWorkflowsToValidate.push(
          ...(await workflowBroker.getAllWorkflows(
    ...
    for (let workflowKey in allWorkflowsToValidate) {
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cworkflowNameIs + workflowName,
      );
      ...
      for (const element of allWorkflowsToValidate) {
        ...
        await loggers.consoleLog(
          namespacePrefix + functionName,
          msg.csecondTierWorkflowNameIs + secondTierWorkflowName,
        );
      ...
      if (numberOfDuplicatesFound > 1) {
        ...
        let duplicateWorkflowCountMessage =
          msg.cDuplicateWorkflowCountIs + numberOfDuplicatesFound;
        duplicateWorkflowCountMessage = await colorizer.colorizeMessageSimple(
          duplicateWorkflowCountMessage,
          blackColorArray,
          true,
        );
        duplicateWorkflowCountMessage = await colorizer.colorizeMessageSimple(
        ...
        let duplicateWorkflowMessage =
          msg.cDuplicateWorkflowNameIs + workflowName;
        duplicateWorkflowMessage = await colorizer.colorizeMessageSimple(
  ...
  await configurator.setConfigurationSetting(
    wrd.csystem,
    cfg.cpassedAllWorkflowDuplicateChecks,
...
async function runAllValidations(inputData, inputMetaData) {
  ...
  if (validationTypesInputArray.length > 0) {
    for (let validationTypesKey in validationTypesInputArray) {
      ...
      if (
        ...
        console.log(
          msg.cWarningUserEnteredValidationDateTypeMessage02 +
            validValidationUserTypes.join(bas.cComa + bas.cSpace),
        );
      }
    } // End-for (let validationTypesKey in validationTypesInputArray)
    ...
    if (validationTypesConfirmedArray.length > 0) {
      ...
      console.log(
        msg.cWarningUserEnteredValidationDataTypeMessage02 +
          validValidationUserTypes.join(bas.cComa + bas.cSpace),
      );
    }
  } else {
  ...
  if (validUserEntry === true) {
    let constantsValidationCommandToQueue =
      cmd.cvalidateConstants + bas.cSpace + validationTypesConfirmedList;
    let commandAliasesValidationCommandToQueue =
      cmd.cvalidateCommandAliases + bas.cSpace + validationTypesConfirmedList;
    let workflowsValidationCommandToQueue =
      cmd.cvalidateWorkflows + bas.cSpace + validationTypesConfirmedList;
    if (
      (await configurator.getConfigurationSetting(
        wrd.csystem,
        cfg.clogAllCommands,
      ...
      await stack.push(
        sys.cSystemCommandLog,
        constantsValidationCommandToQueue,
      );
      await stack.push(
        sys.cSystemCommandLog,
        commandAliasesValidationCommandToQueue,
      );
      await stack.push(
        sys.cSystemCommandLog,
        workflowsValidationCommandToQueue,
      );
    }
