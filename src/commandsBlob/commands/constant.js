  ...
  if (
    ...
    if (inputData.length === 0) {
      while (validEntry === false) {
        ...
        userDefinedConstant = await ruleBroker.processRules(
          [bas.cGreaterThan, ""],
          [biz.cprompt],
        );
        validEntry = await ruleBroker.processRules(
          [userDefinedConstant, ""],
          [biz.cisConstantValid],
        );
        if (validEntry === false) {
      ...
      userDefinedConstant = await ruleBroker.processRules(
        [inputData, ""],
        [recombineArrayInputRule],
      );
    }
    // userDefinedConstant is:
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cuserDefinedConstantIs + userDefinedConstant,
    );
    ...
    let doesConstantExist = await ruleBroker.processRules(
      [userDefinedConstant, ""],
      [biz.cdoesConstantExist],
    );
    if (doesConstantExist === true) {
      let constantType = await ruleBroker.processRules(
        [userDefinedConstant, false],
        [biz.cgetConstantType],
      );
    ...
    let wordCount = await ruleBroker.processRules(
      [userDefinedConstant, ""],
      [biz.cgetWordCountInString],
    );
    // wordCount is:
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cwordCountIs + wordCount,
    );
    // Now begin the fulfillment algorithm.
    if (wordCount > 1) {
      let wordsArray = await ruleBroker.processRules(
        [userDefinedConstant, ""],
        [biz.cgetWordsArrayFromString],
      );
      for (const element of wordsArray) {
        let optimizedWordConstantDefinition = await ruleBroker.processRules(
          [element.trim(), element.trim()],
          [constantsFulfillmentSystemRule],
        );
        // Optimized constant definition for word:
        console.log(
          msg.cOptimizedConstantDefinitionForWord +
            bas.cc +
            element +
            bas.cSpace +
            bas.cEqual +
      ...
      returnData[1] = await ruleBroker.processRules(
        [userDefinedConstant, userDefinedConstant],
        [constantsFulfillmentSystemRule],
      );
    ...
    errorMessage =
      msg.cconstantsGeneratorMessage3 + msg.cconstantsGeneratorMessage4;
  ...
  if (
    ...
    if (inputData.length === 0) {
      while (validEntry === false) {
        ...
        userDefinedConstantList = await ruleBroker.processRules(
          [bas.cGreaterThan, ""],
          [biz.cprompt],
        );
        validEntry = await ruleBroker.processRules(
          [userDefinedConstantList, ""],
          [biz.cisConstantValid],
        );
        if (validEntry === false) {
      ...
      userDefinedConstantList = await ruleBroker.processRules(
        [inputData, ""],
        [biz.crecombineStringArrayWithSpaces],
      );
    }
    // userDefinedConstantList is:
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cuserDefinedConstantListIs + userDefinedConstantList,
    );
    if (userDefinedConstantList.includes(bas.cComa) === true) {
      // userDefinedConstantList contains comas
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cuserDefinedConstantListContainsComas,
      );
      let userDefinedConstantsListArray = userDefinedConstantList.split(
        bas.cComa,
      );
      // userDefinedConstantsListArray is:
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cuserDefinedConstantsListArrayIs +
          JSON.stringify(userDefinedConstantsListArray),
      );
      if (userDefinedConstantsListArray.length > 1) {
        for (const element of userDefinedConstantsListArray) {
          let commandToQueue1 =
            cmd.cconstantsGenerator + bas.cSpace + element.trim();
          if (
            (await configurator.getConfigurationSetting(
              wrd.csystem,
              cfg.clogAllCommands,
        ...
        let commandToQueue2 =
          cmd.cconstantsGenerator +
          bas.cSpace +
          userDefinedConstantsListArray[0].trim();
        if (
          (await configurator.getConfigurationSetting(
            wrd.csystem,
            cfg.clogAllCommands,
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cuserDefinedConstantsListDoesNotContainComas,
      );
      let commandToQueue3 =
        cmd.cconstantsGenerator + bas.cSpace + userDefinedConstantList.trim();
      if (
        (await configurator.getConfigurationSetting(
          wrd.csystem,
          cfg.clogAllCommands,
...
async function constantsPatternRecognizer(inputData, inputMetaData) {
  ...
  if (
    ...
    if (userDefinedConstantList.includes(bas.cComa) === true) {
      ...
      wordsArray = await ruleBroker.processRules(
        [userDefinedConstantList, ""],
        [biz.cgetWordsArrayFromString],
      );
    }
    ...
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cwordsArrayIs + JSON.stringify(wordsArray),
    );
    commonPatternsArray = await ruleBroker.processRules(
      [wordsArray, ""],
      [biz.csearchForPatternsInStringArray],
    );
    // commonPatternsArray is:
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.ccommonPatternsArrayIs + JSON.stringify(commonPatternsArray),
    );
    ...
    let newConstantsList = await ruleBroker.processRules(
      [commonPatternsArray, ""],
      [biz.cvalidatePatternsThatNeedImplementation],
    );
    let constantsPatternGenerationSetting =
      await configurator.getConfigurationSetting(
        wrd.csystem,
        cfg.cenableConstantsPatternGeneration,
      );
    if (constantsPatternGenerationSetting === true) {
      let commandToQueue =
        cmd.cconstantsGeneratorList + bas.cSpace + newConstantsList;
      if (
        (await configurator.getConfigurationSetting(
          wrd.csystem,
          cfg.clogAllCommands,
...
async function evaluateConstant(inputData, inputMetaData) {
  ...
  if (inputData && inputData.length > 1) {
    if (
      ...
      returnData[1] = await ruleBroker.processRules(
        [inputData[1], ""],
        [biz.cgetConstantActualValue],
      );
      console.log(
        inputData[1] + bas.cSpace + bas.cEqual + bas.cSpace + returnData[1],
      );
    } else {
