...
async function printDataHive(inputData, inputMetaData) {
  ...
  let returnData = [true, {}];
  let printDataHiveToLogFileConfigSetting =
    await configurator.getConfigurationSetting(
      wrd.csystem,
      cfg.cprintDataHiveToLogFile,
    );
  ...
  await loggers.consoleLog(
    namespacePrefix + functionName,
    msg.clogFilePathAndNameIs + logFilePathAndName,
  );
  if (inputData && inputData[1].includes(bas.cDot) === true) {
    ...
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cdataHivePathArrayIs + JSON.stringify(dataHivePathArray),
    );
    ...
    for (let i = 0; i < dataHivePathArray.length; i++) {
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cBEGIN_ithIteration + i,
      );
      leafDataHiveElement = leafDataHiveElement[dataHivePathArray[i]];
      // contents of leafDataHiveElement is:
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.ccontentsOfLeafDataHiveElementIs +
          JSON.stringify(leafDataHiveElement),
      );
      // END i-th iteration:
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cEND_ithIteration + i,
      );
    ...
    if (printDataHiveToLogFileConfigSetting === true) {
      await loggers.printMessageToFile(
        logFilePathAndName,
        inputData[1] +
          bas.cSpace +
          msg.ccontentsAre +
    ...
    if (D[inputData[1]] !== undefined) {
      ...
      if (printDataHiveToLogFileConfigSetting === true) {
        await loggers.printMessageToFile(
          logFilePathAndName,
          msg.ccontentsOfDare + JSON.stringify(D),
        );
      }
      returnData[1] = D;
    }
...
async function printDataHiveAttributes(inputData, inputMetaData) {
  ...
  if (inputData && inputData.length > 1) {
    if (inputData[1].includes(bas.cDot) === true) {
      ...
      if (leafDataHiveElement && leafDataHiveElement.length > 0) {
        ...
        leafDataHiveElementKeys2.forEach((key2) => {
          if (attributeName.toLowerCase() === wrd.ckey) {
            ...
            // entity is:
            console.log(
              msg.centityIs + JSON.stringify(leafDataHiveElement[key2]),
            );
          } else {
      ...
      errorMessage =
        msg.cprintDataHiveAttributesMessage1 +
        msg.cprintDataHiveAttributesMessage2;
...
async function changeSetting(inputData, inputMetaData) {
  ...
  if (inputData && inputData.length === 3) {
    ...
    let parentDataObject = await ruleBroker.processRules(
      [dataPath, ""],
      [biz.cgetNamespacedDataObject],
    );
    // parentDataObject BEFORE mutation is:
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cparentDataObjectBeforeMutationIs + JSON.stringify(parentDataObject),
    );
    ...
    await loggers.consoleLog(
      namespacePrefix + functionName,
      msg.cparentDataObjectAfterMutationIs + JSON.stringify(parentDataObject),
    );
    // Persist the change back to the D-data structure.
    await ruleBroker.processRules(
      [dataPath, parentDataObject],
      [biz.csetNamespacedDataObject],
    );
    returnData[1] = true;
  } else {
