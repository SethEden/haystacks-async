...
async function businessRulesMetrics(inputData, inputMetaData) {
  ...
  if (businessRuleMetricsEnabled === true) {
    ...
    for (
      let i = 0;
      i < (await stack.length(cfg.cbusinessRulesNamesPerformanceTrackingStack));
      i++
    ) {
      ...
      let currentBusinessRuleName =
        D[cfg.cbusinessRulesNamesPerformanceTrackingStack][i];
      for (
        let j = 0;
        j < (await stack.length(cfg.cbusinessRulesPerformanceTrackingStack));
        j++
      ) {
        if (
          D[cfg.cbusinessRulesPerformanceTrackingStack][j][wrd.cName] ===
          currentBusinessRuleName
        ) {
          businessRuleCounter = businessRuleCounter + 1;
          // businessRuleCounter is:
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cbusinessRuleCounterIs + businessRuleCounter,
          );
          businessRulePerformanceSum =
            businessRulePerformanceSum +
            D[cfg.cbusinessRulesPerformanceTrackingStack][j][sys.cRunTime];
          // businessRulePerformanceSum is:
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cbusinessRulePerformanceSumIs + businessRulePerformanceSum,
          );
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cDoneBusinessRulePerformanceSumIs + businessRulePerformanceSum,
      );
      average = businessRulePerformanceSum / businessRuleCounter;
      // average is:
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.caverageIs + average,
      );
      // Now go back through them all so we can compute the standard deviation.
      for (
        let k = 0;
        k < (await stack.length(cfg.cbusinessRulesPerformanceTrackingStack));
        k++
      ) {
        if (
          D[cfg.cbusinessRulesPerformanceTrackingStack][k][wrd.cName] ===
          currentBusinessRuleName
        ) {
          businessRulePerformanceStdSum =
            businessRulePerformanceStdSum +
            math.pow(
              D[cfg.cbusinessRulesPerformanceTrackingStack][k][sys.cRunTime] -
                average,
              2,
            );
          ...
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.cbusinessRulePerformanceStdSumIs +
              businessRulePerformanceStdSum,
          );
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cDoneBusinessRulePerformanceStdSumIs +
          businessRulePerformanceStdSum,
      );
      standardDev = math.sqrt(
        businessRulePerformanceStdSum / businessRuleCounter,
      );
      // standardDev is:
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cstandardDevIs + standardDev,
      );
      ...
      await stack.push(cfg.cbusinessRulesPerformanceAnalysisStack, {
    ...
    await loggers.consoleTableLog(
      "",
      D[cfg.cbusinessRulesPerformanceAnalysisStack],
      [wrd.cName, wrd.cAverage, sys.cStandardDeviation],
    );
    ...
    if (
      (await configurator.getConfigurationSetting(
        wrd.csystem,
        cfg.cclearBusinessRulesPerformanceDataAfterAnalysis,
      )) === true
    ) {
...
async function commandMetrics(inputData, inputMetaData) {
  ...
  if (commandMetricsEnabled === true) {
    ...
    for (
      let i = 0;
      i < (await stack.length(cfg.ccommandNamesPerformanceTrackingStack));
      i++
    ) {
      commandCounter = 0;
      ...
      for (
        let j = 0;
        j < (await stack.length(cfg.ccommandsPerformanceTrackingStack));
        j++
      ) {
        if (
          D[cfg.ccommandsPerformanceTrackingStack][j][wrd.cName] ===
          currentCommandName
        ) {
          ...
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.ccommandCounterIs + commandCounter,
          );
          commandPerformanceSum =
            commandPerformanceSum +
            D[cfg.ccommandsPerformanceTrackingStack][j][sys.cRunTime];
          // commandPerformanceSum is:
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.ccommandPerformanceSumIs + commandPerformanceSum,
          );
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cDoneCommandPerformanceSumIs + commandPerformanceSum,
      );
      ...
      for (
        let k = 0;
        k < (await stack.length(cfg.ccommandsPerformanceTrackingStack));
        k++
      ) {
        if (
          D[cfg.ccommandsPerformanceTrackingStack][k][wrd.cName] ===
          currentCommandName
        ) {
          commandPerformanceStdSum =
            commandPerformanceStdSum +
            math.pow(
              D[cfg.ccommandsPerformanceTrackingStack][k][sys.cRunTime] -
                average,
              2,
            );
          ...
          await loggers.consoleLog(
            namespacePrefix + functionName,
            msg.ccommandPerformanceStdSumIs + commandPerformanceStdSum,
          );
      ...
      await loggers.consoleLog(
        namespacePrefix + functionName,
        msg.cDoneCommandPerformanceStdSumIs + commandPerformanceStdSum,
      );
      ...
      await stack.push(cfg.ccommandsPerformanceAnalysisStack, {
    ...
    await loggers.consoleTableLog(
      "",
      D[cfg.ccommandsPerformanceAnalysisStack],
      [wrd.cName, wrd.cAverage, sys.cStandardDeviation],
    );
    returnData[1] = await ruleBroker.processRules(
      [D[cfg.ccommandsPerformanceAnalysisStack], ""],
    ...
    if (
      (await configurator.getConfigurationSetting(
        wrd.csystem,
        cfg.cclearCommandPerformanceDataAfterAnalysis,
      )) === true
    ) {
